...
def _measure_text_size(text: str, font) -> tuple:
...
def make_subtitle_frame(text: str, width: int, height: int, fontsize: int = 40) -> ImageClip:
    img = Image.new("RGBA", (width, height), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)

    # Font — try Liberation Sans (installed in Docker), fall back gracefully
    try:
        font = ImageFont.truetype(
            "/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf", fontsize
        )
    except Exception:
        try:
...
def build_subtitle_clips(script: str, total_duration: float, width: int, height: int):
