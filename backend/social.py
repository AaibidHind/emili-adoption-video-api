...
import jsonimport osimport timeimport urllib.parsefrom dataclasses import dataclass, asdictfrom datetime import datetime, timezonefrom pathlib import Pathfrom typing import Any, Dict, Optional, Tuple

import requestsfrom googleapiclient.discovery import buildfrom googleapiclient.http import MediaFileUploadfrom google.auth.transport.requests import Requestfrom google.oauth2.credentials import Credentials
...
@dataclassclass SocialPostResult:platform: strsuccess: boolmessage: strvideo_path: strtitle: strdescription: strextra: Optional[Dict[str, Any]] = None

LOG_DIR = Path("out/social_logs")

def log_result(res: SocialPostResult) -> None:LOG_DIR.mkdir(parents=True, exist_ok=True)ts = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")safe_platform = res.platform.replace("/", "").replace("\", "")out_path = LOG_DIR / f"{ts}{safe_platform}.json"payload = asdict(res)with out_path.open("w", encoding="utf-8") as f:json.dump(payload, f, ensure_ascii=False, indent=2, default=str)
...
def _public_url_for_file(video_path: Path) -> Optional[str]:base = os.getenv("SOCIAL_PUBLIC_BASE_URL")if not base:return None
...
def _build_youtube_client() -> Tuple[Optional[Any], Optional[str]]:client_id = os.getenv("YOUTUBE_CLIENT_ID")client_secret = os.getenv("YOUTUBE_CLIENT_SECRET")refresh_token = os.getenv("YOUTUBE_REFRESH_TOKEN")
...
def _upload_to_youtube(video_path: Path, title: str, description: str) -> SocialPostResult:youtube, err = _build_youtube_client()if youtube is None:res = SocialPostResult("youtube", False, err or "Could not build YouTube client.", str(video_path), title, description)_log_result(res)return res
...
def _post_to_facebook_page_via_url(video_path: Path, title: str, description: str) -> SocialPostResult:page_id = os.getenv("FACEBOOK_PAGE_ID")access_token = os.getenv("FACEBOOK_PAGE_ACCESS_TOKEN")
...
def _post_to_instagram_via_url(video_path: Path, title: str, description: str) -> SocialPostResult:ig_user_id = os.getenv("INSTAGRAM_BUSINESS_ACCOUNT_ID")access_token = os.getenv("FACEBOOK_PAGE_ACCESS_TOKEN") or os.getenv("INSTAGRAM_ACCESS_TOKEN")video_url = _public_url_for_file(video_path)
...
def _post_to_tiktok_via_url(video_path: Path, title: str, description: str) -> SocialPostResult:# 1. Récupération du tokentoken_file = Path("tokens.json")access_token = Noneif token_file.exists():try:tokens = json.loads(token_file.read_text(encoding="utf-8"))tk_data = tokens.get("tiktok", {})access_token = tk_data.get("access_token") or tk_data.get("data", {}).get("access_token")except Exception:pass
...
def post_to_platform(platform: str, video_path: Path, title: str, description: str) -> Dict[str, Any]:normalized = platform.lower().strip()
